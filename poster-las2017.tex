...
%\usepackage{url}
\title{\parbox{\linewidth}{\centering \textbf{A Heuristic Method for Large-Scale Cognitive-Diagnostic Computerized Adaptive Testing}}}
...
\begin{columns} % Blocks will be placed into columns
    ...
    \column{.55}
    \block[roundedcorners=40]{Context: Cognitive-Diagnostic Adaptive Testing}{
        \begin{itemize}
        ...
        \innerblock[]{Item Response Theory}{
            \[ \Pr(\textnormal{student}_{\textnormal{level}=\theta} \textnormal{ \textsl{VS} question}_{\textnormal{difficulty}=d} = correct) \triangleq \frac1{1 + e^{-(\theta - d)}} \]
            \hfill $\Rightarrow$ \textnormal{ no feedback}
        }
        \vspace{1cm}
        \innerblock[]{DINA, Attribute Hierarchy Model}{
            \[ \Pr(\textnormal{student}_{\textnormal{KC}^\textnormal{*}} \textnormal{ \textsl{VS} question}_{\begin{subarray}{l}\textnormal{KC}\\\textnormal{slip}=s\\\textnormal{guess}=g\end{subarray}} = correct) \triangleq \left\{\begin{array}{cl}
1 - s & \textnormal{if all question}_\textnormal{KC} \textnormal{ mastered by student}_\textnormal{KC}\\
g & \textnormal{otherwise.}
\end{array}\right. \]
            \hfill $\Rightarrow$ \textnormal{ not scalable}
        }
    ...
    \block{Computerized Adaptive Testing (CAT)}{
        ...
        \vspace{1cm}
        \includegraphics[width=\linewidth]{figures/adaptive.pdf}
    }

    \block{Domain: Dependency Graph}{
        \begin{itemize}
            \item Each validation triggers the validation of parents.
            \item Each non-validation triggers the non-validation of children.
        \end{itemize}
        \includegraphics[width=\linewidth]{figures/prerequis.png}
    }

    \block{Example of CAT tree}{
        \includegraphics[width=\linewidth]{figures/cat.png}
    }
